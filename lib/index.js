...
exports['default'] = () => {
  return {
    ...
    /*path, meta*/
      ...
      this.write(name).newline().newline();
    },
    ...
    async captureLogs(testId, level, message, time, attachment) {
      try {
        if (this.reporter.displayDebugLogs) process.stdout.write(`\n[Test ${testId}] Capturing log: ${message} \n`);
        if (!this.reporter.liveReporting) process.logs.push({
          type: level,
          log: message,
          file: attachment,
          time: new Date().valueOf()
        });else await this.reportLogs(testId, level, message, time, attachment);
        return message;
      } catch (error) {
    ...
    async reportLogs(testId, level, message, time, attachment) {
      if (message !== undefined) {
        ...
        const isException = isJSON && JSON.parse(message).errMsg !== undefined; //If the log is a stacktrace, and we want to focus on printing the error message itself.

        if (isException) message = JSON.parse(message).errMsg; //If the log is a JS Object
        else if (isJSON) message = JSON.parse(message);else if (typeof message === 'object') message = `"${message}"`;
    ...
    async reportTestDone(name, testRunInfo) {
      ...
      if (testRunInfo.screenshots) {
        testRunInfo.screenshots.forEach(async (screenshot, idx) => {
          await this.captureLogs(this.reporter.test.id, 'debug', `Taking screenshot (${name}-${idx}.png)`, new Date().valueOf(), {
            name: `${name}-${idx}.png`,

...
class API {
  ...
  async finishTestItem(projectName, testItemId, options) {
    try {
      ...
      return this.handleResponse(await this.client.put(`/v1/${projectName}/item`, {
        "issues": [{
      ...
      })); //@Sahil  Shukla - Added code for Changing the status as 'To Investigate'
    } catch (error) {
      return this.handleError(error);
    }
  }
  ...
  buildMultiPartStream(jsonPart, filePart, boundary) {
    const eol = '\r\n';
    const bx = `--${boundary}`;
    const buffers = [Buffer.from(bx + eol + 'Content-Disposition: form-data; name="json_request_part"' + eol + 'Content-Type: application/json' + eol + eol + eol + JSON.stringify(jsonPart) + eol), Buffer.from(bx + eol + 'Content-Disposition: form-data; name="file"; filename="' + filePart.name + '"' + eol + 'Content-Type: ' + filePart.type + eol + eol), Buffer.from(filePart.content, 'base64'), Buffer.from(`${eol + bx}--${eol}`)];
  ...
  async sendLog(projectName, options) {
    try {
      if (typeof options.message !== 'string') options.message = `${options.message}`;
      if (options.file) {
  ...
  parseError(error) {
    ...
    return `${errorMessage}${responseData && typeof responseData === 'object' ? `: ${JSON.stringify(responseData)}` : ''}`;
  }
